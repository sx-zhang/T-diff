...
class Sem_Exp_Env_Agent(MultiObjectGoal_Env):
    ...
    def reset(self):
        ...
        if args.visualize or args.print_images:
            ...
            save_path = "{}/eps_{:04d}_{}.mp4".format(thread_dir, self.episode_no, info["success"]) 
    ...
    def _plan(self, planner_inputs):
        ...
        if stop and planner_inputs["found_goal"] == 1:            
            planner_t = FMMPlanner(traversible=np.ones((480,480)))
            if self.goal_names[self.active_goal_ix]=="bed" or self.goal_names[self.active_goal_ix]=="couch" or self.goal_names[self.active_goal_ix]=="chair":
                selem_t = skimage.morphology.disk(3)
                goal_t = cv2.erode(planner_inputs['goal'], selem_t)
                selem_t = skimage.morphology.disk(3)
                goal_t = cv2.dilate(goal_t, selem_t)
                if goal_t.max()==0:
                    goal_t = planner_inputs['goal']
            else:
                selem_t = skimage.morphology.disk(1)
                goal_t = cv2.erode(planner_inputs['goal'], selem_t)
                selem_t = skimage.morphology.disk(1)
                goal_t = cv2.dilate(goal_t, selem_t)
                if goal_t.max()==0:
            ...
            planner_t.set_multi_goal(goal_t)
            cur_loc_0, cur_loc_1 = start_y * 20 - gx1, start_x *20 - gy1
            cur_loc_0, cur_loc_1 = max(cur_loc_0, 0), max(cur_loc_1, 0)
            cur_loc_0, cur_loc_1 = min(cur_loc_0, 479), min(cur_loc_1, 479)
            curr_distance = planner_t.fmm_dist[int(cur_loc_0), int(cur_loc_1)] / 20.0
            offset = 0.05
            if curr_distance <= 1 + self.args.success_distance + offset:
                action = 0  # Stop
            else:
                action = 1
        # ================================================================
    ...
    def _visualize(self, inputs):
        ...
        # ==============================GT_map
        self.vis_image[50:530,1660:2140] = sem_map_gt
        ...
        agent_arrow_3 = vu.get_contour_points(pos, origin=(1660, 50), size=10)
        color = (
