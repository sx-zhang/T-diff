...
def data_check(scene_name):
    ...
    for i, eps in enumerate(eps_data):
        ...
        cv2.imwrite('..img/test.png',np.flipud(mx_vis), [int(cv2.IMWRITE_JPEG_QUALITY), 95])
        cv2.imwrite('..img/dist_map.png',np.flipud(dist_map_vis), [int(cv2.IMWRITE_JPEG_QUALITY), 95])
        pass
    # save_data
